...
analysis_dir = glob.glob(os.path.join(home, 'data/analysis/',(project +'*')))[0]
...
for r in range(setup_df.shape[0]):
    ...
    if err == 'False':
        ...
        opts = ("{\'" + '\', \''.join(cons_incl) + "\'}, {\'" + '\', \''.join(group_names) + "\'}, {" + str(list_suffixes) + "}, {" + str(analysis_type) + "}, \'" + results_dir + "\', {\'" + result_suffix + "\'}")
