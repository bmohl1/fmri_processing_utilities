...
home = '/home'
intervening_dir = 'data/analysis/brianne'
project = 'priming'
...
analysis_dir = glob.glob(os.path.join(home, intervening_dir,(project +'*')))[0]
...
########################################################################
if len(demo_file) > 1:
    print('Detected multiple versions of the study demographics.\nAssuming the first is the correct sheet: {}'.format(demo_file[0]))
df = pd.DataFrame(pd.read_excel(demo_file[0])).astype(str).sort_values(by=['subject_id'])
...
setup_file = glob.glob(os.path.join(analysis_dir, '*setup*xlsx')) #this will enable multiple filters to be setup with full brain power and traceable record
if len(setup_file) > 1:
    print('Detected multiple versions of the study demographics.\nAssuming the first is the correct sheet: {}'.format(setup_file[0]))
setup_df = pd.DataFrame(pd.read_excel(setup_file[0])).dropna(how = 'all')
setup_df.columns = [x.lower() for x in setup_df.columns] #to attempt to normalize names for ensuing functions
...
for r in range(setup_df.shape[0]-5, setup_df.shape[0]):
    ...
    analysis_dict = setup_df.iloc[r].dropna().to_dict() #since each analysis might have different columns that are empty, do the col drop here.
    task = analysis_dict['task'].strip()
    results_dir = glob.glob(os.path.join(home, intervening_dir,(project +'*'), 'results', task))[0]
    #results_dir = 'dummy_file'
    groups = [x.strip() for x in analysis_dict['groups'].split(',')]
    print(len(groups))
    conditions = list([k for k,v in analysis_dict.items() if k in study_filters])
    ...
    for p in range(len(groups)):
        ...
        [filter_dict, filters, result_suffix] = fs.define_filter_list(**input_dict) #normal_responder = responder[k])
        ...
        name = analysis_dict['groups'].split(',')[p].strip() #By setting this new variable before defining the object, we can sneakily avoid truly dynamic variable naming and duplicating an object.
    ...
    if err == 'False':
        ...
        grp_list_suffixes = [groups[x].filter for x in range(len(groups))] #to compbine with "group" in filename
        #print(list_suffixes)
        grp_list_suffixes = ('\'' + '\', \''.join(map(str,grp_list_suffixes)) + '\'')
        #print(list_suffixes)
        cons_incl = [x.strip() for x in analysis_dict['cons_incl'].split(',')]
        analysis_type = [x.strip() for x in analysis_dict['analysis_type'].split(',')]
        res_basename = [x.strip() for x in analysis_dict['result_basename'].split(',')]
        if 'suffix' in analysis_dict:
            result_suffix = (result_suffix + analysis_dict['suffix'])
        timept_comparison = [x.strip() for x in analysis_dict['timept_comparison'].split(',')]
