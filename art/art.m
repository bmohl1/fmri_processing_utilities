...
mvmt_diff_thresh = 1.5;     %scan-to-scan subject motion threshold - CHANGED TO 1.5 from 2.0 BMM
...
if ~isempty(sess_file)
    subj_dir = fileparts(output_dir); % 160321 BMM
    raw_dir = strcat(subj_dir,filesep,'raw'); %160321 BMM - clumsy, but it isn't reading the cfg file... 
    finder = (strfind(subj_dir,'/')); % grabs the filepath for the subject's main folder - 160321 BMM
    task = subj_dir(finder(6)+1:end); % assumes a task folder is the "main folder" 160321 BMM
    basename =  subj_dir(finder(5)+1:finder(6)-1); % assumes the subject's folder is above the task, main folder
    art_graph_save = strcat(raw_dir,filesep,basename,'_',task,'_art_graphs_autosave'); %160321 BMM
    
    %savefile_Callback(hObject, eventdata, handles)
    fprintf('Saving graph to %s\n',art_graph_save); %160321 BMM
    saveas(gcf,art_graph_save,'eps'); %Added 050713 BMM
 %%   
    summary_file = [strcat(raw_dir,filesep,basename,'_',task,'_art_summary_stats')]; %160321 BMM
        fp = fopen(summary_file,'w');
     if fp ~= -1   % 160404 BMM     
        fprintf(fp,'%5s%10s%10s%10s%11s%10s%9s%10s\n',' ','x','y','z',' pitch','roll','yaw','norm');
        fprintf(fp,'%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f\n',mv_stats(1,1:3),mv_stats(1,4:6),mv_stats(1,32));
        fprintf(fp,'%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f\n',mv_stats(2,1:3),mv_stats(2,4:6),mv_stats(2,32));
        fprintf(fp,'%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f\n\n\n',mv_stats(3,1:3),mv_stats(3,4:6),mv_stats(3,32));
        fclose(fp);
        save(summary_file,'fp') % 160404 BMM
        fprintf('Saved %s\n', summary_file) % 160404 BMM
     else % 160404 BMM
         disp('unable to establish summary file.') % 160404 BMM
     end % 160404 BMM
end %BMM
...
for j=1:num_sess,
    ...
    R1(lidx,:)=eye(length(lidx)); %BMM Look at this for eliminating the seventh, "summary" regressor
    R2=cat(2,R1,M{j});
    R2=cat(2,R2,mv_data(cur_idx+(1:size(g{j},1)),32));
...
if loadfromfile
    ...
    while ~strcmp(s,'end')
        ...
        elseif strcmp(s,'subj_dir:')  %160321 BMM
            output_dir = art_fullfile(fscanf(fp,'%s',1)); %160321 BMM
