function varargout = art_bmm(varargin)
...
% ------------------------
z_thresh = 9.0;             %global signal threshold
mvmt_thresh = 2.0;          %absolute subject motion threshold
rotat_thresh = .05;         %absolute subject rotation threshold
mvmt_diff_thresh = 1.0;     %scan-to-scan subject motion threshold - CHANGED TO 1.5 from 2.0 BMM
...
if ~isempty(sess_file) % read config
    disp('Reading the parameter file')
    [num_sess,global_type_flag,drop_flag,motionFileType,motion_threshold,global_threshold,use_diff_motion,use_diff_global,use_norms,SPMfile,mask_file,output_dir,subj_dir,raw_dir,P,M] = ...
        read_art_sess_file(sess_file); %BMM add subj_dir and raw_dir 160321 for saving?
    %% The GUI walk-through follows
else
...
for sess=1:num_sess
    ...
    % --------------------------------------------------
    notcoregistered=length(VY{sess})>1&&any(any(VY{sess}(2).mat~=VY{sess}(1).mat));
...
%ELIMINATED BMM 160324 % 7     : euclidean norm of raw movement parameters
% 7-12  : scan-to-scan differences in raw movement parameters
% 13-30 : 6 control points trajectories (placed on center of faces of bounding box; x,y,z coordinates for each control point)
% 31    : composite measure: euclidean norm of control point trajectories
% 32-49 : scan-to-scan differences in 6 control points trajectories
% 50    : composite measure: max scan-to-scan movement across the 6 control points

mv_data=[mv_data,zeros([size(mv_data,1),50-size(mv_data,2)])];
...
for i=1:size(mv_data,1)
    ...
    mv_data(i,13:30)=temp*res';
end
cur_sess_start=0;
for sess=1:num_sess
    n=length(g{sess}(:,1));
    mv_data(cur_sess_start+(1:n));
    mv_data(cur_sess_start+(2:n),7:12)  = diff(mv_data(cur_sess_start+(1:n),1:6),1,1);
    mv_data(cur_sess_start+(1:n),31)=sqrt(mean(abs(detrend(mv_data(cur_sess_start+(1:n),13:30),'constant')).^2,2));
    mv_data(cur_sess_start+(2:n),32:49)=diff(mv_data(cur_sess_start+(1:n),13:30),1,1);
    mv_data(cur_sess_start+(2:n),50)=max(sqrt(sum(reshape(abs(mv_data(cur_sess_start+(2:n),32:49)).^2,[n-1,3,6]),2)),[],3);
    %BMM 160324 to eliminate 7th column: mv_data(cur_sess_start+(1:n),7) = sqrt(sum(abs(mv_data(cur_sess_start+(1:n),1:3)).^2,2));
    %BMM 160324 to eliminate 7th column: mv_data(cur_sess_start+(2:n),7:12)  = diff(mv_data(cur_sess_start+(1:n),1:6),1,1);
    %BMM 160324 to eliminate 7th column: mv_data(cur_sess_start+(1:n),32)=sqrt(mean(abs(detrend(mv_data(cur_sess_start+(1:n),14:31),'constant')).^2,2));
    %BMM 160324 to eliminate 7th column: mv_data(cur_sess_start+(2:n),33:50)=diff(mv_data(cur_sess_start+(1:n),14:31),1,1);
    %BMM 160324 to eliminate 7th column: mv_data(cur_sess_start+(2:n),51)=max(sqrt(sum(reshape(abs(mv_data(cur_sess_start+(2:n),33:50)).^2,[n-1,3,6]),2)),[],3);
...
fprintf('%7s%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f\n','mean ',mv_stats(1,1:3),mv_stats(1,4:6),mv_stats(1,31));
fprintf('%7s%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f\n','stdv ',mv_stats(2,1:3),mv_stats(2,4:6),mv_stats(2,31));
fprintf('%7s%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f\n\n\n','max ',mv_stats(3,1:3),mv_stats(3,4:6),mv_stats(3,31));
...
if ~isempty(stats_file)
    ...
    if fp ~= -1
        ...
        fprintf(fp,'%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f\n',mv_stats(1,1:3),mv_stats(1,4:6),mv_stats(1,31));
        fprintf(fp,'%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f\n',mv_stats(2,1:3),mv_stats(2,4:6),mv_stats(2,31));
        fprintf(fp,'%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f%10.4f\n\n\n',mv_stats(3,1:3),mv_stats(3,4:6),mv_stats(3,31));
        fclose(fp);
    end
end
...
set(handles.figure1,'resizefcn','art_bmm(''showOptions_Callback'',gcbo,[],guidata(gcbo))'); %changed to 'art_bmm' 160324 BMM
...
if ~isempty(sess_file)
     [~,~,~,~,~,~,~,~,~,~,~,output_dir,subj_dir,raw_dir,P,M] = ...
        read_art_sess_file(sess_file); %BMM add subj_dir and raw_dir 160321 for saving?
   
    [~,task ] =fileparts(output_dir);
    %task = subj_dir(finder(6)+1:end); % assumes a task folder is the "main folder" 160321 BMM
    subj = textscan(subj_dir,'%s','Delimiter','/');
    basename =  subj{1,1}{end};
    art_graph_save = strcat(output_dir, filesep, basename, '_', task, '_art_graphs_autosave'); %160321 BMM
    
    %savefile_Callback(hObject, eventdata, handles)
    fprintf('Saving graph to %s\n',art_graph_save); %160321 BMM
    saveas(gcf,art_graph_save,'tiff'); %Added 050713 BMM
end %BMM
%savefile_Callback_SaveGraphs(handles, output_dir); %Commented 050713 BMM - Must be from an older release 160321 BMM
...
set(gca,'XTickLabel',[]);
legend('pitch', 'roll', 'yaw', 'Location', 'East');
h = gca;
set(h,'Ygrid','on');
...
if option_motion>0 % diff_motion
    ...
    mv_data = getappdata(handles.mvthresh,'mv_data');
    tmp = mv_data(:,1:6);
    mv_data(:,1:6) = mv_data(:,7:12);
    mv_data(:,7:12) = tmp;
    tmp = mv_data(:,13:31);
    mv_data(:,13:31) = mv_data(:,32:50);
    mv_data(:,32:50) = tmp;
    setappdata(handles.mvthresh,'mv_data',mv_data);
end
...
if isempty(plotdata)||~isfield(plotdata,'figurehandle')||~isequal(plotdata.figurehandle,handles.figure1), % note: keeps only one file at a time in memory (to speed up processing while working on the same art project, while avoiding running out of memory when having too many art windows open)
    ...
    dir_order=zeros(1,3);
    [~,dir_order(3)]=max(abs(temp(3,:)));dir_order(3)=dir_order(3)*sign(temp(3,dir_order(3)));
...
if option>2
    ...
    for n1=1:numel(out_idx),
        i=out_idx(n1);
        if plotdata.notcoregistered, temp=spm_get_data(plotdata.VY(i),pinv(plotdata.VY(i).mat)*plotdata.xyz);
...
[S,v] = listdlg('PromptString','What would you like to save?',...
    ...
    'ListSize',[160,120], ...
    'ListString',{'Outliers','Motion Statistics','Graphs','SPM regressors','Analysis mask','Voxel-wise Variability','Voxel-wise SNR'});
...
for j=1:num_sess,
    ...
    R2=cat(2,R1,M{j});
    R2=cat(2,R2,mv_data(cur_idx+(1:size(g{j},1)),31));
    ...
    try
        ...
        try
            ...
            [filename, pathname] =uiputfile({'*.mat'},['Save session ',num2str(j),' regressor:'],fullfile(datafiles_path,['art_regression_outliers_',datafiles_name,'.mat']));
            filename = fullfile(char(pathname), char(filename));
            R=R1;save(filename ,'R','-mat'); %#ok<NASGU>
            [filename, pathname] =uiputfile({'*.mat'},['Save session ',num2str(j),' regressor:'],fullfile(datafiles_path,['art_regression_outliers_and_movement_',datafiles_name,'.mat']));
            filename = fullfile(char(pathname), char(filename));
...
for noption=1:numel(option)
    switch option(noption)
        case 1, b=Mask; filename='art_mask.img'; filetype='uint8'; filedescription='analysis mask';
        case 2, b=Mask.*Data_Std; filename='art_ResStd.img'; filetype='float32'; filedescription='Voxel-wise variability';
        case 3, b=Mask.*Data_SNR; filename='art_SNR.img'; filetype='float32'; filedescription='Voxel-wise SNR';
    end
    V=plotdata.VY1;
    V.fname=fullfile(output_dir,filename);
    V.descrip=filedescription;
    V.dt=[spm_type(filetype) spm_platform('bigend')];
    spm_write_vol(V,b);
...
% ohinds 2008-04-23
% -----------------------------------------------------------------------
function [num_sess,global_type_flag,drop_flag,motionFileType,motion_threshold,global_threshold,use_diff_motion,use_diff_global,use_norms,SPMfile,mask_file,output_dir,subj_dir,raw_dir,P,M] = read_art_sess_file(sess_file)
...
if loadfromfile
    ...
    while ~strcmp(s,'end')
        ...
        elseif strcmp(s,'subj_dir:')  %160321 BMM
            subj_dir = art_fullfile(fscanf(fp,'%s',1)); %160321 BMM
        elseif strcmp(s,'raw_dir:') %160321 BMM
            raw_dir = art_fullfile(fscanf(fp,'%s',1)); %160321 BMM
        end
        s = fscanf(fp,'%s',1);
    end
    ...
    if isfield(sess_file,'subj_dir'), output_dir=sess_file.subj_dir; end %160321 BMM
    if isfield(sess_file,'raw_dir'), output_dir=sess_file.raw_dir; end %160321 BMM
...
UpdateGlobal(hObject, eventdata, handles,0.95);
UnionOutliers(hObject, eventdata, handles);
UpdateSummaryplot(hObject, eventdata, handles);
end
...
UpdateMovement(hObject, eventdata, handles,0.95);
UnionOutliers(hObject, eventdata, handles);
UpdateSummaryplot(hObject, eventdata, handles);
end
...
UpdateRotation(hObject, eventdata, handles,0.95)
UnionOutliers(hObject, eventdata, handles);
UpdateSummaryplot(hObject, eventdata, handles);
end
