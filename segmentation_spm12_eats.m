function [subjs] = segmentation_spm12_eats(subjs, subj_redo_segment, child_temp)
...
for nSubj = 1:length(pth_subjdirs);
    ...
    subj_prefix = (subj(1:end-3)); %Need to back off one character to accomodate finding the T1
        ...
        if eq(check_never_processed, 1) || eq(subj_redo_segment,1)
            if strfind(subj_t1_dir, subj);
                ...
                if isempty(arrayfun(@(y) isempty(y.name), c1_img)) || eq(subj_redo_segment,1)
                    ...
                    savefile = strcat(subj_t1_dir,filesep,'segmentation_',subj,'.mat');
